	...
	struct device_attribute *attr, const char *buf, size_t cnt)
{
	...
	char *str = NULL;
	char *p_buf, *tmp_buf;
	int i = 0, j = 0, led_color[LED_MAX_NUM] = {0}, led_order[LED_MAX_NUM] = {0};
	int led_groups_num = led->pdata->led->led_groups_num;
	ssize_t ret = -EINVAL;
	...
	for (i = 0; i < LED_MAX_NUM; i++) {
		led_order[i] = led->pdata->led->led_allocation_order[i];
	}
	...
	if (i == led_groups_num) {
		...
		for (j = 0; j < led_groups_num; j++) {
			led->pdata->color[j] = led_color[led_order[j]];
		}
//		led_esd_color[led->id][0] = led->pdata->color[0];
//		led_esd_color[led->id][1] = led->pdata->color[1];
//		led_esd_color[led->id][2] = led->pdata->color[2];
//		led_esd_color[led->id][3] = led->pdata->color[3];
		mutex_unlock(&led->pdata->led->lock);
...
//		return ret;
	...
	if (ret) {
		...
		for (i = 0; i < aw210xx->led_groups_num; i++) {
			aw210xx->led_allocation_order[i] = led_order[i] - 1;
		}
