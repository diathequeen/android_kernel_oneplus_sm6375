...
static struct proc_dir_entry *rpm_debugfs_dir;
...
static const struct proc_ops rsc_ops = {
	.proc_write = rsc_ops_write,
};

static int __init rpm_smd_debugfs_init(void)
{
	rpm_debugfs_dir = proc_mkdir("rpm_send_msg", NULL);
	if (!rpm_debugfs_dir)
		return -ENOMEM;

	if (!proc_create("message", 0200, rpm_debugfs_dir, &rsc_ops))
		return -ENOMEM;
