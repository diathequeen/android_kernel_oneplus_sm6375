	...
	struct device *dev)
{
	...
	const char *switch_to_fast_charger_support = "fsa4480,switch-to-fast-charger-support";
	int ret = 0;
	int rc = 0;
	...
	rc = of_property_read_u32(dNode, switch_to_fast_charger_support, &fsa_priv->switch_to_fast_charger_support);
	if (rc) {
		pr_info("%s: %s in dt node is missing\n", __func__, switch_to_fast_charger_support);
		fsa_priv->switch_to_fast_charger_support = 1;
	}
	return ret;
}
#endif /* OPLUS_ARCH_EXTENDS */
...
tcp_register_finish:
	...
	if (fsa_priv->vendor == WAS4783 && fsa_priv->switch_to_fast_charger_support == 1) {
...
static void fsa4480_remove(struct i2c_client *i2c)
{
	...
	#if IS_ENABLED(CONFIG_TCPC_CLASS)
	if (fsa_priv->tcpc) {
		ret = unregister_tcp_dev_notifier(fsa_priv->tcpc, &fsa_priv->nb, TCP_NOTIFY_TYPE_ALL);
		if (ret < 0) {
