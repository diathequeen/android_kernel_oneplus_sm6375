...
#if IS_ENABLED(CONFIG_OPLUS_FEATURE_SF_SLIDE_BOOST)
	bool sf_boost_on_big = slide_rt_boost(task);
	bool boost_on_big = rt_boost_on_big() || sf_boost_on_big;
#else
...
#if IS_ENABLED(CONFIG_OPLUS_FEATURE_FRAME_BOOST)
#if IS_ENABLED(CONFIG_OPLUS_FEATURE_SF_SLIDE_BOOST)
	if (is_fbg_task(task) && frame_boost_enabled() && !sf_boost_on_big)
		order_index = 0;
#else
...
retry:
	...
	for (cluster = 0; cluster < num_sched_clusters; cluster++) {
		for_each_cpu_and(cpu, lowest_mask, &cpu_array[order_index][cluster]) {
			 ...
			 */
#if IS_ENABLED(CONFIG_OPLUS_FEATURE_SF_SLIDE_BOOST)
			if (*best_cpu != -1 && test_task_ux(cpu_rq(cpu)->curr) && !sf_boost_on_big)
#else
